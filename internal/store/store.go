...
func (s *Store) SetDiscordWebhookURL(webhookURL string) error {
	s.mu.Lock()
	s.settings.DiscordWebhookURL = webhookURL
...
func (s *Store) RecordExecution(id string, result model.ExecuteResult) error {
	...
	if len(entryLogs) > 15 {
		entryLogs = entryLogs[len(entryLogs)-15:]
	}
	s.logs[id] = entryLogs
...
func (s *Store) snapshotLocked() persistedState {
	...
	return persistedState{Entries: entries, Logs: logs, Settings: s.settings}
}
